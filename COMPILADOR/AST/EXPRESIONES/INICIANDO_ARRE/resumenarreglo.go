...
type ResumenArreglo struct {
	Exp1 interfaces.Expresion
	Exp2 interfaces.Expresion
}

func Nresumenarre(e1 interfaces.Expresion, e2 interfaces.Expresion) ResumenArreglo {
	return ResumenArreglo{Exp1: e1, Exp2: e2}
}

func (ini ResumenArreglo) Ejecutar_Expresion(ent *entorno.Entorno) simbolos.Valor {
	data, tipo := ini.Obteniendo_Valores(ent)

	return simbolos.Valor{Valor: data, Tipo: tipo}
}

func (ini ResumenArreglo) Obteniendo_Valores(ent *entorno.Entorno) (interface{}, simbolos.TipoExpresion) {
	...
	for i := 0; i < data.Len(); i++ {
		...
		if valor_datico.Tipo != simbolos.ARRAY {
			if i == 0 {
	...
	return objtval, simbolos.ARRAY
}

func (ini ResumenArreglo) Compilar_Expresion(ent *entorno.Entorno, gen *generador.Generador_C3D) simbolos.ValoresC3D {
