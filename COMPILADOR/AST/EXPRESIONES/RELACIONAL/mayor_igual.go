...
func Nopmayorigaul(oi interfaces.Expresion, od interfaces.Expresion, l int, c int) OpMayorIgual {
	mayor := OpMayorIgual{OpIzq: oi, OpDer: od, Linea: l, Columna: c}
	return mayor
}

func (mayorigual OpMayorIgual) Ejecutar_Expresion(ent *entorno.Entorno) simbolos.Valor {
	...
	var operador_izq simbolos.Valor = mayorigual.OpIzq.Ejecutar_Expresion(ent)
	var operador_der simbolos.Valor = mayorigual.OpDer.Ejecutar_Expresion(ent)
	...
	if tipo_dominante == simbolos.INTEGER {
		...
		//ERROR
		t := time.Now()
		reportes.Nerror("No es posible realizar la operacion: Mayor Igual", ent.Nombre_Entorno, strconv.Itoa(mayorigual.Linea), strconv.Itoa(mayorigual.Columna), t.Format("2006-01-02 15:04:05"))
	}
...
func (mayorigual OpMayorIgual) Compilar_Expresion(ent *entorno.Entorno, gen *generador.Generador_C3D) simbolos.ValoresC3D {
	//		INTEGER				FLOAT			  BOOLEAN		CHAR	   		TEXTO		&TEXTO		NULL
	relacional_domiante := [7][7]simbolos.TipoExpresion{
		//INTEGER
		{simbolos.INTEGER, simbolos.NULL, simbolos.NULL, simbolos.INTEGER, simbolos.NULL, simbolos.NULL, simbolos.NULL},
	...
	var operador_izq simbolos.ValoresC3D = mayorigual.OpIzq.Compilar_Expresion(ent, gen)
	var operador_der simbolos.ValoresC3D = mayorigual.OpDer.Compilar_Expresion(ent, gen)
