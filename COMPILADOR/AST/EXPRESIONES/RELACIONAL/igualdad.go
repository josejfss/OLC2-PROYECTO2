...
func Nopigualdad(oi interfaces.Expresion, od interfaces.Expresion, l int, c int) OpIgualdad {
	igual := OpIgualdad{OpIzq: oi, OpDer: od, Linea: l, Columna: c}
	return igual
}

func (igualdad OpIgualdad) Ejecutar_Expresion(ent *entorno.Entorno) simbolos.Valor {
	...
	var operador_izq simbolos.Valor = igualdad.OpIzq.Ejecutar_Expresion(ent)
	var operador_der simbolos.Valor = igualdad.OpDer.Ejecutar_Expresion(ent)
	...
	if tipo_dominante == simbolos.INTEGER {
		...
		//ERROR
		t := time.Now()
		reportes.Nerror("No es posible realizar la operacion: Igualdad", ent.Nombre_Entorno, strconv.Itoa(igualdad.Linea), strconv.Itoa(igualdad.Columna), t.Format("2006-01-02 15:04:05"))
	}
...
func (igualdad OpIgualdad) Compilar_Expresion(ent *entorno.Entorno, gen *generador.Generador_C3D) simbolos.ValoresC3D {
	//		INTEGER				FLOAT			  BOOLEAN		CHAR	   		TEXTO		&TEXTO		NULL
	relacional_domiante := [7][7]simbolos.TipoExpresion{
		//INTEGER
		{simbolos.INTEGER, simbolos.NULL, simbolos.NULL, simbolos.INTEGER, simbolos.NULL, simbolos.NULL, simbolos.NULL},
	...
	var operador_izq simbolos.ValoresC3D = igualdad.OpIzq.Compilar_Expresion(ent, gen)
	var operador_der simbolos.ValoresC3D = igualdad.OpDer.Compilar_Expresion(ent, gen)
	...
	if tipo_dominante == simbolos.INTEGER {
		...
		rigual := "/*--INGRESANDO OPERACION IGUALDAD--*/\n"
		rigual += "if (" + operador_izq.Valor + " " + "==" + " " + operador_der.Valor + ") { goto " + etiqueta_verdadera + "; }\ngoto " + etiqueta_falsa + ";"
		rigual += "\n/*--SALIENDO OPERACION IGUALDAD--*/\n"
