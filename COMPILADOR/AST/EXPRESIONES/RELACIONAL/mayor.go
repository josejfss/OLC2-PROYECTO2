...
func Nopmayor(oi interfaces.Expresion, od interfaces.Expresion, l int, c int) OpMayor {
	mayor := OpMayor{OpIzq: oi, OpDer: od, Linea: l, Columna: c}
	return mayor
}

func (mayor OpMayor) Ejecutar_Expresion(ent *entorno.Entorno) simbolos.Valor {
	...
	var operador_izq simbolos.Valor = mayor.OpIzq.Ejecutar_Expresion(ent)
	var operador_der simbolos.Valor = mayor.OpDer.Ejecutar_Expresion(ent)
	...
	if tipo_dominante == simbolos.INTEGER {
		...
		//ERROR
		t := time.Now()
		reportes.Nerror("No es posible realizar la operacion: Mayor", ent.Nombre_Entorno, strconv.Itoa(mayor.Linea), strconv.Itoa(mayor.Columna), t.Format("2006-01-02 15:04:05"))
	}
...
func (mayor OpMayor) Compilar_Expresion(ent *entorno.Entorno, gen *generador.Generador_C3D) simbolos.ValoresC3D {
	//		INTEGER				FLOAT			  BOOLEAN		CHAR	   		TEXTO		&TEXTO		NULL
	relacional_domiante := [7][7]simbolos.TipoExpresion{
		//INTEGER
		{simbolos.INTEGER, simbolos.NULL, simbolos.NULL, simbolos.INTEGER, simbolos.NULL, simbolos.NULL, simbolos.NULL},
	...
	var operador_izq simbolos.ValoresC3D = mayor.OpIzq.Compilar_Expresion(ent, gen)
	var operador_der simbolos.ValoresC3D = mayor.OpDer.Compilar_Expresion(ent, gen)
	...
	if tipo_dominante == simbolos.INTEGER {
		...
		rmayor := "/*--INGRESANDO OPERACION MAYOR--*/\n"
		rmayor += "if (" + operador_izq.Valor + " " + ">" + " " + operador_der.Valor + ") { goto " + etiqueta_verdadera + "; }\ngoto " + etiqueta_falsa + ";"
		rmayor += "\n/*--SALIENDO OPERACION MAYOR--*/\n"
