...
func Nopmenor(oi interfaces.Expresion, od interfaces.Expresion, l int, c int) OpMenor {
	menor := OpMenor{OpIzq: oi, OpDer: od, Linea: l, Columna: c}
	return menor
}

func (menor OpMenor) Ejecutar_Expresion(ent *entorno.Entorno) simbolos.Valor {
	...
	var operador_izq simbolos.Valor = menor.OpIzq.Ejecutar_Expresion(ent)
	var operador_der simbolos.Valor = menor.OpDer.Ejecutar_Expresion(ent)
	...
	if tipo_dominante == simbolos.INTEGER {
		...
		//ERROR
		t := time.Now()
		reportes.Nerror("No es posible realizar la operacion: Menor", ent.Nombre_Entorno, strconv.Itoa(menor.Linea), strconv.Itoa(menor.Columna), t.Format("2006-01-02 15:04:05"))
	}
...
func (menor OpMenor) Compilar_Expresion(ent *entorno.Entorno, gen *generador.Generador_C3D) simbolos.ValoresC3D {
	//		INTEGER				FLOAT			  BOOLEAN		CHAR	   		TEXTO		&TEXTO		NULL
	relacional_domiante := [7][7]simbolos.TipoExpresion{
		//INTEGER
		{simbolos.INTEGER, simbolos.NULL, simbolos.NULL, simbolos.INTEGER, simbolos.NULL, simbolos.NULL, simbolos.NULL},
	...
	var operador_izq simbolos.ValoresC3D = menor.OpIzq.Compilar_Expresion(ent, gen)
	var operador_der simbolos.ValoresC3D = menor.OpDer.Compilar_Expresion(ent, gen)
