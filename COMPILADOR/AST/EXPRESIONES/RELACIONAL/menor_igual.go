...
func Nopmenorigual(oi interfaces.Expresion, od interfaces.Expresion, l int, c int) OpMenorIgual {
	menorigual := OpMenorIgual{OpIzq: oi, OpDer: od, Linea: l, Columna: c}
	return menorigual
}

func (menorigual OpMenorIgual) Ejecutar_Expresion(ent *entorno.Entorno) simbolos.Valor {
	...
	var operador_izq simbolos.Valor = menorigual.OpIzq.Ejecutar_Expresion(ent)
	var operador_der simbolos.Valor = menorigual.OpDer.Ejecutar_Expresion(ent)
	...
	if tipo_dominante == simbolos.INTEGER {
		...
		//ERROR
		t := time.Now()
		reportes.Nerror("No es posible realizar la operacion: Menor Igual", ent.Nombre_Entorno, strconv.Itoa(menorigual.Linea), strconv.Itoa(menorigual.Columna), t.Format("2006-01-02 15:04:05"))
	}
...
func (menorigual OpMenorIgual) Compilar_Expresion(ent *entorno.Entorno, gen *generador.Generador_C3D) simbolos.ValoresC3D {
	//		INTEGER				FLOAT			  BOOLEAN		CHAR	   		TEXTO		&TEXTO		NULL
	relacional_domiante := [7][7]simbolos.TipoExpresion{
		//INTEGER
		{simbolos.INTEGER, simbolos.NULL, simbolos.NULL, simbolos.INTEGER, simbolos.NULL, simbolos.NULL, simbolos.NULL},
	...
	var operador_izq simbolos.ValoresC3D = menorigual.OpIzq.Compilar_Expresion(ent, gen)
	var operador_der simbolos.ValoresC3D = menorigual.OpDer.Compilar_Expresion(ent, gen)
	...
	if tipo_dominante == simbolos.INTEGER {
		...
		rmenorigual := "/*--INGRESANDO OPERACION MENOR IGUAL--*/\n"
		rmenorigual += "if (" + operador_izq.Valor + " " + "<=" + " " + operador_der.Valor + ") goto " + etiqueta_verdadera + ";\ngoto " + etiqueta_falsa + ";"
		rmenorigual += "\n/*--SALIENDO OPERACION MENOR IGUAL--*/\n"
